...
def test_valid_request_reaches_application_and_preserves_identity():
    ...
    assert seen[0].command.correlation_id == "corr-1"


def test_missing_request_id_is_rejected_without_application_call():
    value = payload(); del value["request_id"]; calls = []
    class App:
        def handle(self, request):
            calls.append(request); raise AssertionError("must not be called")
...
def test_application_failure_is_sanitized():
    class App:
        def handle(self, request): raise RuntimeError("secret internal detail")
    response = HTTPTransportAdapter(App()).handle_json(json.dumps(payload()).encode())
    body = json.loads(response.body)
...
def test_application_error_result_maps_deterministically():
    class App:
        def handle(self, request): return TransportResponse("req-1", "corr-1", "cmd-1", False, "unknown_command", "Unknown command type: x")
    response = HTTPTransportAdapter(App()).handle_json(json.dumps(payload()).encode())
...
def test_response_serialization_failure_is_transport_error():
    class App:
        def handle(self, request): return TransportResponse("req-1", "corr-1", "cmd-1", True, "ok", "done", object())
    response = HTTPTransportAdapter(App()).handle_json(json.dumps(payload()).encode())
    body = json.loads(response.body)
    assert response.status == 500
...
def test_application_boundary_authentication_precedes_authorization_and_dispatch():
    ...
    class Auth:
        def authenticate(self, request):
            order.append("authenticate"); return AuthenticationResult(True, SecurityContext("user", "test", frozenset(), {}))
    class Authz:
        def authorize(self, context, command): order.append("authorize"); return AuthorizationDecision(True)
...
def test_unauthorized_request_never_reaches_dispatcher():
    ...
    class Authz:
        def authorize(self, context, command): return AuthorizationDecision(False, "authorization_denied", "Denied.")
    response = HTTPTransportAdapter(ApplicationBoundary(dispatcher, authorizer=Authz())).handle_json(json.dumps(payload()).encode())
...
def test_authentication_failure_never_reaches_authorization_or_dispatch():
    ...
    class Auth:
        def authenticate(self, request):
            order.append("authenticate"); return AuthenticationResult(False, None, "authentication_failed", "No identity.")
    class Authz:
        def authorize(self, context, command): order.append("authorize"); return AuthorizationDecision(True)
    response = HTTPTransportAdapter(ApplicationBoundary(dispatcher, Auth(), Authz())).handle_json(json.dumps(payload()).encode())
...
def test_observer_failure_does_not_fail_valid_execution():
    ...
    class Observer:
        def record(self, event): raise RuntimeError("observer unavailable")
    response = HTTPTransportAdapter(ApplicationBoundary(dispatcher, observability=Observer())).handle_json(json.dumps(payload()).encode())
    assert response.status == 200
...
def test_invalid_command_schema_is_mapped_as_bad_request():
    dispatcher = CommandDispatcher(); dispatcher.register("agent.get_status", lambda command: {})
...
def test_http_server_can_be_created_without_external_service():
    server = HTTPTransportAdapter(ApplicationBoundary(CommandDispatcher())).create_server(port=0)
    try: assert server.server_address[1] > 0
