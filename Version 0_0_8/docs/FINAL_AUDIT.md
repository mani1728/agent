...
PR #40 merge commit on `main`: `a181b6364eb87597de10b0a781bbf45892d6cec9`.
...
PR #40 was merged into `main`. Tag `v0.0.8` was published by the owner and points to `ec0585624c4defd30c8d368be8756a33e2d3caa5`. GitHub Release `MT5 Agent v0.0.8` is published with `MT5Agent-v0.0.8.exe`; the release asset digest matches the canonical SHA-256 above.

The difference between the tag/release commit and the final `main` merge commit is an intentional historical state: the tag/release were created before closure documentation and final merge processing. The published tag/release must not be moved or rewritten.
