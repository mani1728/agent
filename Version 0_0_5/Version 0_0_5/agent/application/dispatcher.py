...
class UnsupportedSchemaError(ApplicationError):
    """Raised when a command schema version is not supported."""


class CommandDispatcher:
    """Validate commands and dispatch them to transport-neutral handlers."""
    ...
    def dispatch(self, command: Command) -> CommandResult:
        try:
            ...
            return CommandResult(
                command.command_id if isinstance(command, Command) else "",
                command.correlation_id if isinstance(command, Command) else "",
                False,
                "invalid_command",
                str(exc),
            )
